...
CLEANUP_FILE="$HP_ROOT/scripts/cleanup_urltest.uc"
HOMEPROXY_UC="$HP_ROOT/scripts/homeproxy.uc"

printf '[ZN-HomeProxy] Target package: %s\n' "$HP_PATH"

HOMEProxy_JS="$HP_PATH/htdocs/luci-static/resources/homeproxy.js"
...
for REQUIRED_FILE in \
    ...
    if [ ! -f "$REQUIRED_FILE" ]; then
        echo "[ERROR] Required HomeProxy file missing: $REQUIRED_FILE"
        echo "[ERROR] Refusing to build a mixed/incomplete package."
        exit 1
    fi
done

if ! grep -q "getServiceStatus(instance)" "$HOMEProxy_JS" || \
   ! grep -q "getBuiltinFeatures()" "$HOMEProxy_JS"; then
    echo "[ERROR] homeproxy.js is not the VIKINGYFY-compatible module expected by this patch."
    echo "[ERROR] Required exports: getServiceStatus(instance), getBuiltinFeatures()"
    exit 1
fi

if [ ! -f "$CLEANUP_FILE" ]; then
    echo "[ERROR] cleanup_urltest.uc not found in the target HomeProxy package:"
    echo "  $CLEANUP_FILE"
    echo "[ERROR] Refusing to build a mixed-version package."
    exit 1
fi

if ! grep -q "export function addECHDNS" "$HOMEPROXY_UC" || \
   ! grep -q "export function reconcileUrltestNodes" "$HOMEPROXY_UC"; then
    echo "[ERROR] homeproxy.uc is not compatible with the target generator/cleanup_urltest.uc."
    echo "[ERROR] Required exports: addECHDNS, reconcileUrltestNodes"
    exit 1
fi
...
+		so = s.option(form.ListValue, 'domain_strategy', _('Domain strategy'),
+			_('If set, the requested domain name will be resolved to IP before routing.'));
+		for (let i in hp.dns_strategy)
+			so.value(i, hp.dns_strategy[i]);
+
+		so = s.option(form.ListValue, 'default_outbound', _('Default outbound'),
...
+		so.rmempty = false;
+
+		so = s.option(form.ListValue, 'default_outbound_dns', _('Default outbound DNS'),
...
+			} : null,
+			detour: cfg.outbound === 'direct-out' ? null : get_custom_outbound(cfg.outbound)
+		});
+	});
+
...
# ============================================================
# 4.5. sing-box >= 1.13 final legacy inbound-field hardening
#     必须放在最终 JSON writefile() 之前，防止上游后续代码重新写入。
# ============================================================
ensure_modern_inbound_fields() {
    local tmp="$1"

    if grep -q "ZN: final sing-box 1.13+ inbound cleanup" "$GEN_FILE"; then
        echo "[ZN-HomeProxy] Final legacy inbound cleanup already present; verifying position."
    else
        cp -a "$GEN_FILE" "$BACKUP_DIR/generate_client.final-cleanup-original"

        python3 - "$GEN_FILE" "$tmp" <<'PY'
import sys
...
dst = Path(sys.argv[2])
s = src.read_text()
marker = "/* ZN: final sing-box 1.13+ inbound cleanup */"
block = """/* ZN: final sing-box 1.13+ inbound cleanup */
for (let i = 0; i < length(config.inbounds); i++) {
        delete config.inbounds[i].sniff;
        delete config.inbounds[i].sniff_override_destination;
        delete config.inbounds[i].sniff_timeout;
        delete config.inbounds[i].domain_strategy;
        delete config.inbounds[i].udp_disable_domain_unmapping;
}
...
if marker in s:
    dst.write_text(s)
    raise SystemExit(0)
anchor = "if (!writefile(RUN_DIR + '/sing-box-c.json.new',"
if s.count(anchor) != 1:
    raise SystemExit(f"final writefile anchor count is {s.count(anchor)}, expected 1")
dst.write_text(s.replace(anchor, block + anchor, 1))
PY
        mv -f "$tmp" "$GEN_FILE"
    fi
...
anchor = "if (!writefile(RUN_DIR + '/sing-box-c.json.new',"
if s.count(marker) != 1:
    raise SystemExit("final cleanup marker count is not 1")
if s.count(anchor) != 1:
    raise SystemExit("final writefile anchor count is not 1")
if s.index(marker) > s.index(anchor):
    raise SystemExit("final cleanup is not before writefile")
for field in [
    "sniff", "sniff_override_destination", "sniff_timeout",
    "domain_strategy", "udp_disable_domain_unmapping"
]:
    if f"delete config.inbounds[i].{field};" not in s:
        raise SystemExit(f"missing cleanup: {field}")
PY

    echo "[ZN-HomeProxy] Final legacy inbound cleanup verified before JSON serialization."
}

LEGACY_INBOUND_TMP="$(mktemp)"
if ! ensure_modern_inbound_fields "$LEGACY_INBOUND_TMP"; then
    rm -f "$LEGACY_INBOUND_TMP"
    echo "[ERROR] Failed to install final legacy inbound cleanup."
    exit 1
fi
rm -f "$LEGACY_INBOUND_TMP"
...
# ============================================================
# 5. Adaptive private SRS binding
#
# Current VIKINGYFY defines geoip-cn/geosite-cn as local rule-sets.
# We redirect those definitions to private_srs. If a future upstream
# adds geosite-noncn in the same local-rule-set function, it is handled
# too. Any other structure fails closed instead of guessing.
# ============================================================
patch_private_mainland_srs() {
    local tmp="$1"
    awk '
    BEGIN { in_func=0; depth=0; tag=""; changed=0; seen_noncn=0 }
    /^function[[:space:]]+add_mainland_rule_sets[[:space:]]*\(/ {
        in_func=1
        depth=0
        if (index($0, "geosite-noncn") > 0) expected=3
    }
    {
        if (in_func) {
            opens=gsub(/\{/, "{")
            ...
            if ($0 ~ /tag:[[:space:]]*\x27geoip-cn\x27/) tag="geoip-cn"
            else if ($0 ~ /tag:[[:space:]]*\x27geosite-cn\x27/) tag="geosite-cn"
            else if ($0 ~ /tag:[[:space:]]*\x27geosite-noncn\x27/) { tag="geosite-noncn"; seen_noncn=1 }

            if (tag == "geoip-cn" && $0 ~ /path:[[:space:]]*/) {
                sub(/path:[[:space:]]*.*/, "path: HP_DIR + " sprintf("%c",39) "/private_srs/cn.srs" sprintf("%c",39))
                changed++
                tag=""
            } else if (tag == "geosite-cn" && $0 ~ /path:[[:space:]]*/) {
                sub(/path:[[:space:]]*.*/, "path: HP_DIR + " sprintf("%c",39) "/private_srs/geosite-geolocation-cn.srs" sprintf("%c",39))
                changed++
                tag=""
            } else if (tag == "geosite-noncn" && $0 ~ /path:[[:space:]]*/) {
                sub(/path:[[:space:]]*.*/, "path: HP_DIR + " sprintf("%c",39) "/private_srs/geosite-geolocation-!cn.srs" sprintf("%c",39))
                changed++
                tag=""
            }
    ...
    END { if (changed != (2 + seen_noncn)) exit 7 }
    ' "$GEN_FILE" > "$tmp"
}

if grep -q "path: HP_DIR + '/private_srs/cn.srs'" "$GEN_FILE" && \
   grep -q "path: HP_DIR + '/private_srs/geosite-geolocation-cn.srs'" "$GEN_FILE" && \
   { ! grep -q "tag: 'geosite-noncn'" "$GEN_FILE" || grep -q "path: HP_DIR + '/private_srs/geosite-geolocation-!cn.srs'" "$GEN_FILE"; }; then
    echo "[ZN-HomeProxy] Private mainland SRS binding already present."
else
    SRS_BACKUP="$BACKUP_DIR/generate_client.srs-original"
    cp -a "$GEN_FILE" "$SRS_BACKUP"
    if ! patch_private_mainland_srs "$GEN_FILE.tmp"; then
        rm -f "$GEN_FILE.tmp"
        echo "[ERROR] Current HomeProxy mainland RuleSet structure is unsupported; refusing to guess."
        cp -af "$SRS_BACKUP" "$GEN_FILE"
        exit 1
    fi
    mv -f "$GEN_FILE.tmp" "$GEN_FILE"
fi

if grep -q "tag: 'geosite-noncn'" "$GEN_FILE"; then
    check_noncn_path=1
    echo "[ZN-HomeProxy] Optional geosite-noncn detected and handled."
else
    check_noncn_path=0
fi

# Ensure the third mainland SRS is also available for Custom Routing.
# Current VIKINGYFY does not define geosite-noncn, so add it to the
# existing add_mainland_rule_sets() function without changing the
# upstream routing logic.
if ! grep -q "tag: 'geosite-noncn'" "$GEN_FILE"; then
    if ! grep -q "function[[:space:]]\+add_mainland_rule_sets" "$GEN_FILE"; then
        echo "[ERROR] Cannot safely add geosite-noncn: add_mainland_rule_sets() not found."
        exit 1
    fi
    awk '
    BEGIN { in_func=0; added=0 }
    /^function[[:space:]]+add_mainland_rule_sets[[:space:]]*\(/ { in_func=1 }
    in_func && /^}[[:space:]]*$/ && !added {
        print "\tpush(rule_sets, {"
        print "\t\ttype: " sprintf("%c",39) "local" sprintf("%c",39) ","
        print "\t\ttag: " sprintf("%c",39) "geosite-noncn" sprintf("%c",39) ","
        print "\t\tformat: " sprintf("%c",39) "binary" sprintf("%c",39) ","
        print "\t\tpath: HP_DIR + " sprintf("%c",39) "/private_srs/geosite-geolocation-!cn.srs" sprintf("%c",39)
        print "\t});"
        added=1
        in_func=0
    }
    ...
    ' "$GEN_FILE" > "$GEN_FILE.tmp" || {
        rm -f "$GEN_FILE.tmp"
        echo "[ERROR] Failed to add geosite-noncn safely."
        exit 1
    }
    mv -f "$GEN_FILE.tmp" "$GEN_FILE"
fi
...
# ============================================================
# 6. Strict post-patch validation
# ============================================================
...
check "homeproxy.js getServiceStatus" grep -q "getServiceStatus(instance)" "$HOMEProxy_JS"
check "homeproxy.js getBuiltinFeatures" grep -q "getBuiltinFeatures()" "$HOMEProxy_JS"
check "custom DNS direct-out detour suppression" grep -q "detour: cfg.outbound === 'direct-out' ? null : get_custom_outbound(cfg.outbound)" "$GEN_FILE"
check "config routing options use s" bash -c '! grep -q "so = ss.option(form.ListValue, '''domain_strategy'''" "$1" && ! grep -q "so = ss.option(form.ListValue, '''default_outbound'''" "$1" && ! grep -q "so = ss.option(form.ListValue, '''default_outbound_dns'''" "$1"' _ "$UI_FILE"
...
check "legacy inbound cleanup" grep -q "ZN: final sing-box 1.13+ inbound cleanup" "$GEN_FILE"
check "legacy sniff cleanup" grep -q "delete config.inbounds\[i\]\.sniff;" "$GEN_FILE"
check "legacy sniff_override cleanup" grep -q "delete config.inbounds\[i\]\.sniff_override_destination;" "$GEN_FILE"
check "legacy sniff_timeout cleanup" grep -q "delete config.inbounds\[i\]\.sniff_timeout;" "$GEN_FILE"
check "legacy domain_strategy cleanup" grep -q "delete config.inbounds\[i\]\.domain_strategy;" "$GEN_FILE"
check "legacy udp_unmapping cleanup" grep -q "delete config.inbounds\[i\]\.udp_disable_domain_unmapping;" "$GEN_FILE"
...
check "cleanup_urltest.uc present" test -s "$CLEANUP_FILE"
check "homeproxy.uc addECHDNS export" grep -q "export function addECHDNS" "$HOMEPROXY_UC"
check "homeproxy.uc reconcileUrltestNodes export" grep -q "export function reconcileUrltestNodes" "$HOMEPROXY_UC"
check "private cn.srs binding" grep -q "path: HP_DIR + '/private_srs/cn.srs'" "$GEN_FILE"
check "private geolocation-cn.srs binding" grep -q "path: HP_DIR + '/private_srs/geosite-geolocation-cn.srs'" "$GEN_FILE"
check "geosite-noncn rule-set" grep -q "tag: 'geosite-noncn'" "$GEN_FILE"
check "private geolocation-!cn.srs binding" grep -q "path: HP_DIR + '/private_srs/geosite-geolocation-!cn.srs'" "$GEN_FILE"
